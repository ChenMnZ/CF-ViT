...
def main():
    ...
    args.img_size = max(args.input_size_list)
    args.token_label_size = 2 * args.coarse_stage_size
    ...
    try:
        ...
        for epoch in range(start_epoch, num_epochs):   
            ...
            train_metrics = train_one_epoch(
                ...
                amp_autocast=amp_autocast, loss_scaler=loss_scaler, model_ema=model_ema, mixup_fn=mixup_fn, optimizers=optimizers, input_size_list = args.input_size_list)
            ...
            eval_metrics = validate(model, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast, input_size_list = args.input_size_list)

            if model_ema is not None and not args.model_ema_force_cpu:
                if args.distributed and args.dist_bn in ('broadcast', 'reduce'):
                    distribute_bn(model_ema, args.world_size, args.dist_bn == 'reduce')
                ema_eval_metrics = validate(
                    model_ema.module, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast, log_suffix=' (EMA)',input_size_list = args.input_size_list)
...
def train_one_epoch(
        ...
        loss_scaler=None, model_ema=None, mixup_fn=None, optimizers = None,input_size_list=[112,224]):
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        for i in range(0,len(input_size_list)-1):
            resized_img = F.interpolate(input, (input_size_list[i], input_size_list[i]), mode='bilinear', align_corners=True)
            resized_img = torch.squeeze(resized_img)
            input_list.append(resized_img)  
        input_list.append(input)

        with amp_autocast():
            results = model(input_list)
            loss_ce = loss_fn(results[1], target)
            max_output_detach_softmax = F.softmax(results[1][0].detach(), dim=1)
            kl_layer = torch.nn.KLDivLoss(reduction='batchmean')
            log_softmax_result0 = F.log_softmax(results[0][0], dim=1)
...
def validate_throughput(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix='',input_size_list=[112,224], threshold=0.7):
    batch_time_m = AverageMeter()
    losses_m = AverageMeter()
    top1_m = AverageMeter()
    top5_m = AverageMeter()
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            input_list.append(input)
            starter.record()
            output = model.forward_early_exit(input_list,threshold)
            ender.record()
            torch.cuda.synchronize()
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix='', input_size_list=[112,224]):
    batch_time_m = AverageMeter()
    losses_m = AverageMeter()
    top1_c = AverageMeter()
    top5_c = AverageMeter()
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            loss = loss_fn(results[1], target)

            acc1_c, acc5_c = accuracy(results[0], target, topk=(1, 5))
            acc1_f, acc5_f = accuracy(results[1], target, topk=(1, 5))

            if args.distributed:
                reduced_loss = reduce_tensor(loss.data, args.world_size)
                acc1_c = reduce_tensor(acc1_c, args.world_size)
                acc5_c = reduce_tensor(acc5_c, args.world_size)
                acc1_f = reduce_tensor(acc1_f, args.world_size)
                acc5_f = reduce_tensor(acc5_f, args.world_size)
            else:
            ...
            losses_m.update(reduced_loss.item(), input.size(0))
            top1_c.update(acc1_c.item(), results[0].size(0))
            top5_c.update(acc5_c.item(), results[0].size(0))
            top1_f.update(acc1_f.item(), results[0].size(0))
            top5_f.update(acc5_f.item(), results[0].size(0))
            ...
            if args.local_rank == 0 and (last_batch or batch_idx % args.log_interval == 0):
                ...
                _logger.info(
                    ...
                    'Acc@1_c: {top1_c.val:>7.4f} ({top1_c.avg:>7.4f})  '
                    'Acc@5_c: {top5_c.val:>7.4f} ({top5_c.avg:>7.4f})  '
                    'Acc@1_c: {top1_f.val:>7.4f} ({top1_f.avg:>7.4f})  '
                    'Acc@5_c: {top5_f.val:>7.4f} ({top5_f.avg:>7.4f})'.format(
                        log_name, batch_idx, last_idx, batch_time=batch_time_m,
                        loss=losses_m, top1_c=top1_c, top5_c=top5_c, top1_f=top1_f, top5_f=top5_f,))

    metrics = OrderedDict([('loss', losses_m.avg), ('top1_c', top1_c.avg), (f'top5_c', top5_c.avg),(f'top1_f', top1_f.avg), (f'top5_f', top5_f.avg)])
